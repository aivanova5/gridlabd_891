...
dishwasher::dishwasher(MODULE *module) : residential_enduse(module)
{
	...
	if (oclass==NULL)
	{
		...
		if (gl_publish_variable(oclass,
			...
			PT_complex_array,"state_power",PADDR(state_power), PT_DESCRIPTION, "the power components of each state (row state, column ZIP component)", //should be PT_complex_array
...
int dishwasher::create() 
{
	...
	//state_power.set_name("state_power");
...
#define P 2
	...
	{	
		...
		fsm.from_string("rule:CONTROLSTART->CONTROLEND=$timer>state_duration#CONTROLSTART");
